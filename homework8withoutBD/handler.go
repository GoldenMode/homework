...
// func (h *Handler) GetAllEmployeeDeparment(c *gin.Context) {
...
func (h *Handler) GetEmployeeOffice(c *gin.Context) {
	...
	for id := 1; id <= 1000; id++ {
	employee, err := h.storage.GetOffice(id)
	if err != nil {
		fmt.Printf("failed to get employee %s\n", err.Error())
		c.JSON(http.StatusBadRequest, ErrorResponse{
			Message: err.Error(),
	...
	choose := h.storage.Check(office, employee)
	switch choose {
	case true:
		c.JSON(http.StatusOK, employee)
	default:
